...
Path-scoped rules under `.Codex/rules/` load automatically when you open the matching
...
<!-- Maintainer note: keep this file under ~120 lines. Content Codex can derive by reading
the code (directory trees, constant tables, dependency lists) belongs in docs/, not here.
@imports do NOT reduce context — they expand at launch. Only .Codex/rules/ with `paths:`
frontmatter and per-directory AGENTS.md files load lazily. -->
